...
class SimNetProcessor(object):
    ...
    def get_reader(self, mode):
        ...
        def reader_with_pairwise():
            ...
            if mode == "valid":
                ...
                with open(self.args.test_data_dir,encoding="utf-8") as file:
                    for line in file:
                        query, title, label = line.strip().split("\t")
                        if len(query) == 0 or len(title) == 0 or len(label) == 0 or not label.isdigit() or int(
                                label) not in [0, 1]:
                ...
                with open(self.args.train_data_dir,encoding="utf-8") as file:
                    for line in file:
    ...
    def get_infer_reader(self,query):
        ...
        """
        def reader():
            with open(self.args.infer_data_dir, "r",encoding="utf-8") as file:
                for line in file:
                    title = line.strip()
                    if len(query) == 0 or len(title) == 0:
                        logging.warning("line not match format in test file")
                        continue
                    qry = [self.vocab[word] for word in query.split(" ") if word in self.vocab]
                    title = [self.vocab[word] for word in title.split(" ") if word in self.vocab]
                    if len(qry) == 0:
                        qry = [0]
                    if len(title) == 0:
    ...
    def get_test_label(self):
        ...
        if self.test_label.size == 0:
            labels = []
            with open(self.args.test_data_dir, "r",encoding="utf-8") as f:
                for line in f:
                    labels.append([int(line.strip().split("\t")[-1])])
